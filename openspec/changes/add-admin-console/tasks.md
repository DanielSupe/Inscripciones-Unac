...
- [x] 5.1 Crear la feature de administración con sus hooks de consulta y mutación, y sus query keys
- [x] 5.2 Crear la tabla de usuarios con paginación, búsqueda y filtro por rol
- [x] 5.3 Crear el formulario de alta y edición de cuenta, sin ofrecer nunca el rol de estudiante
- [x] 5.4 Añadir la eliminación con confirmación, y ocultar la acción cuando no procede sin dejar de comprobarlo en el servidor
- [x] 5.5 Añadir el restablecimiento de contraseña, advirtiendo de que hay que comunicarla por fuera del sistema

## 6. Frontend: bandeja y decisión

- [x] 6.1 Crear la bandeja de inscripciones con filtro por estado y por periodo, mostrando el estado del pago y el vencimiento
- [x] 6.2 Crear el detalle de una inscripción, con todo lo declarado y los enlaces a sus documentos
- [x] 6.3 Añadir la acción de tomar para revisión
- [x] 6.4 Añadir la verificación del pago desde el detalle, con su reversión
- [x] 6.5 Añadir aprobar y rechazar, con el motivo obligatorio en el rechazo y la aprobación deshabilitada mientras el pago siga pendiente
- [x] 6.6 Crear la pantalla de periodos académicos con su alta y su edición
- [x] 6.7 Dar destino a las cuatro entradas del menú lateral del administrador, que hasta ahora estaban marcadas como «pronto»

## 7. Pruebas

- [x] 7.1 Probar que un administrador **sí** alcanza la inscripción de cualquier aspirante, que es la primera vez que se ejercita esa excepción en positivo
- [x] 7.2 Probar que aprobar cambia el estado y el rol a la vez, y que el aspirante entra como estudiante después
- [x] 7.3 Probar que aprobar sin el pago verificado se rechaza y no deja nada a medias, ni el estado ni el rol
- [x] 7.4 Probar que rechazar sin motivo se rechaza, y que con motivo el aspirante lo ve
- [x] 7.5 Probar el ciclo completo: enviar, tomar, verificar el pago, rechazar, corregir, reenviar, aprobar
- [x] 7.6 Probar que la cuenta de sistema no se puede eliminar ni degradar, y que su contraseña sí se puede restablecer
- [x] 7.7 Probar que nadie se elimina ni se degrada a sí mismo
- [x] 7.8 Probar que crear o editar una cuenta con rol STUDENT se rechaza
- [x] 7.9 Probar que un aspirante o un estudiante recibe 403 en todas las rutas de administración
- [x] 7.10 Probar el listado: paginación, búsqueda parcial e insensible a mayúsculas, y que las cuentas eliminadas no aparecen
- [x] 7.11 Probar la gestión de periodos: código repetido, fechas incoherentes, y que cambiar la tarifa no altera los recibos ya emitidos
- [x] 7.12 Probar en el frontend la tabla de usuarios y la bandeja, y que aprobar aparece deshabilitado con el pago pendiente

## 8. Documentación y cierre

- [x] 8.1 Actualizar el `README.md`: el ciclo está completo, y cómo entra un administrador a revisar
- [x] 8.2 Ejecutar `pnpm lint`, `pnpm typecheck` y `pnpm test` y dejarlos en verde
- [x] 8.3 Verificar a mano el recorrido completo desde el navegador, con dos sesiones: un aspirante que envía y un administrador que revisa, verifica y aprueba, comprobando que el aspirante pasa a ver la pantalla de estudiante
